...
OTTER_TEST(vm_op_nil) {
  uint8_t instructions[] = {OP_NIL, OP_HALT};
  ...
  OTTER_ASSERT(virtual_machine->stack[0]->type == OTTER_OBJECT_TYPE_NIL);

  OTTER_TEST_END(otter_vm_free(virtual_machine); free_bytecode(bytecode););
}

OTTER_TEST(vm_op_true) {
  uint8_t instructions[] = {OP_TRUE, OP_HALT};
  otter_bytecode *bytecode =
      create_bytecode(OTTER_TEST_ALLOCATOR, instructions, sizeof(instructions));
...
OTTER_TEST(vm_op_false) {
  uint8_t instructions[] = {OP_FALSE, OP_HALT};
  otter_bytecode *bytecode =
      create_bytecode(OTTER_TEST_ALLOCATOR, instructions, sizeof(instructions));
...
OTTER_TEST(vm_op_not_false_to_true) {
  uint8_t instructions[] = {OP_CONSTANT, 0, OP_NOT, OP_HALT};
  otter_bytecode *bytecode =
      create_bytecode(OTTER_TEST_ALLOCATOR, instructions, sizeof(instructions));

  /* Create a false constant that won't be allocated during VM execution */
  otter_object_bool *false_const =
      otter_malloc(OTTER_TEST_ALLOCATOR, sizeof(*false_const));
  false_const->base.type = OTTER_OBJECT_TYPE_BOOL;
  false_const->value = 0;

  bytecode_add_constant(bytecode, (otter_object *)false_const);
  ...
  OTTER_TEST_END(otter_free(OTTER_TEST_ALLOCATOR, false_const);
